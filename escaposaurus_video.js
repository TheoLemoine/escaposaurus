...
function closeOverlay(overlay) {
  ...
  if (gameStart === false) {
    ...
    document.getElementById("overlay-btn").innerHTML =
      "&raquo;&raquo; Continuer l'aventure (JOUER) &laquo;&laquo;";
    gameStart = true;
  } else {
    /*close the overlay*/
    closeIt(overlay);
...
function startGame(overlay) {
  ...
  getJSON(udiskJSONPath, function (err, data) {
    if (err !== null) {
...
function doThePasswordMatch(userTry, foldername) {
  ...
  if (
    isItPasswordProtected(foldername) &&
...
function openContactTxTWindow(vid, bigAvatarHelper) {
  if (mainHintFound) {
    ...
    document
      .getElementById("divcontact-" + vid)
      .classList.add("already-called");
...
function solutionReveal(spid) {
  ...
  if (winState) {
    ...
    var foldername = sequenceFolder[sequenceNumber];
    text = solutionText.password
      .replace("%s1", foldername)
      .replace("%s2", passwordCenter[foldername]);
  }
  sp.innerHTML = text;
  openIt(spid);
}
